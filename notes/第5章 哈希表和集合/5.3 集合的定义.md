...
`Set`表示的是没有重复元素、且不保证顺序（不能保存元素的添加顺序信息）的容器接口。

从特性上来看，`Set`相当于是一个只存储`key`、不存储`value`的`Map`（`Java`中`HashSet`就是使用`HashMap`实现的，内部维护了一个`HashMap`实例变量）。我们可以把`Set`想象成是一个“特殊的`Map`”，这个`Map`只有`key`却没有`value`，所以我们可以用`Set`去除重复的元素。
